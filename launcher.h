...
struct Particle {
	enum Type { LAUNCHING, SPARKLE, TRAIL, FOUNTAIN, DEAD, SNOWFLAKING};
...
public:
	...
	void spawnParticle(glm::vec3 position, glm::vec3 speed, glm::vec4 color, float size, float life, Particle::Type type);//Éú³ÉÁ£×Ó
	void explode(Particle& p);
	void launchFirework();
	void launchFountain();
	void launchSnowflake();
...
private:
	...
	float rocketSize = 0.1f;        /* size of a rocket */
